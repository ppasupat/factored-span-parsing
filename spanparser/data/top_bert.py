...
class Tree(object):
    ...
    @classmethod
    def from_seqlogical(cls, seqlogical, tokenizer):
        ...
        for x in tokens:
            if x[0] == '[':
                ...
                for subtoken in tokenizer.tokenize(x):
                    stack[-1].children.append(Token(subtoken, index))
                    index += 1
...
class TopBertDataset(Dataset):

    def __init__(self, config, meta):
        super().__init__(config, meta)
        self._tokenizer = AutoTokenizer.from_pretrained(config.data.tokenizer)
        self._data = {}
        self._iters = {}
    ...
    def _read_data(self, filename):
        ...
        with open(filename) as fin:
            for index, line in enumerate(fin):
                tree = Tree.from_seqlogical(line.strip(), self._tokenizer)
    ...
    def _gen_vocab(self, examples, config, meta):
        ...
        vocab: vocab from the tokenizer
        ...
        meta.vocab_x = dict(self._tokenizer.get_vocab())
        meta.vocab = [None] * len(meta.vocab_x)
        for x, i in meta.vocab_x.items():
            meta.vocab[i] = x
        # Vocab for the output
        meta.nt = set()
        unary_chains = set()
